...
const getTargetNetwork = (targetChainId) =>
  Number(targetChainId) === 1 ? mainnet : polygon;
...
export function useWalletGateFlow() {
  ...
  const waitForTargetChain = useCallback(
    async (targetChainId) => {
      const started = Date.now();
      ...
      while (Date.now() - started < SWITCH_TIMEOUT_MS) {
        ...
        if (
          currentChain != null &&
          Number(currentChain) === Number(targetChainId)
        ) {
          return true;
        }
        await delay(POLL_MS);
  ...
  const syncWalletToRedux = useCallback(
    async (targetChainId) => {
      ...
      try {
        ...
        console.error("syncWalletToRedux error:", error);
        return false;
      }
    },
    [dispatch, userData],
  ...
  // Reads the wallet's chain directly from window.ethereum (falling back to
  // AppKit/provider state) rather than trusting AppKit's own reactive
  // chainId, which can lag a tick behind a switch performed outside AppKit
  // (e.g. directly in the MetaMask extension). Transaction flows should
  // gate on this instead of `useAppKitNetwork().chainId` alone.
  const isTargetChainMismatched = useCallback(
    async (targetChainId) => {
