...
const TopNftAddQuantiyPurchaseInputBodySection = ({
  ...
  const decrement = () => {
    setQuantity(prev => parseInt(prev) > 1 ? parseInt(prev) - 1 : 1);
  };
  ...
  const handlePurchase = async () => {
    if (address?.toLowerCase() === userData?.address?.toLowerCase()) {
      ...
      if (!liveMismatch) {
        ...
        try {
          const tx = await marketContractWithSigner.BuyFixedPriceItem(itemData?.auctionId, quantity, {
            value: amount
          });
          ...
          const msgData = boughtMessage(userData?.full_name, itemData?.name, itemData?.name, totalcost);
          await sendEmail({
            variables: {
        ...
        <div className="down_side_popup lg:p-4 p-3 bg-white">
          ...
          <div className=" custom-card shadow-move">
            <div className="card-body position-relative">
              ...
              }} src={RedCrossIcon} alt="" className="w-full h-full" />
              </div>
              <div className="">
                <div className="">
                  ...
                  width: "100%"
                    ...
                    <img src="https://plus.unsplash.com/premium_photo-1686727103139-2824d4fa46a3?q=80&w=3327&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" className="rounded-circle img_quantity_top_nft mr-3" alt="Profile Image" style={{
                    objectFit: "cover"
                  }} />
                    <div className="heading_data" style={{
                    width: "90%"
                  }}>
              ...
              <div className="input_go_button_parent">
                {!activeButton ? <>
                    <div className="input_go_button_parent__input_div">
                      <div className="input_go_button_parent__input_div__parent input_go_button_parent__input_div__parent_no_margin">
                        ...
                        {}
                        <input className="quantity_change_input" type="number" style={{
                      width: `${quantity?.toString()?.length > 0 ? quantity?.toString()?.length + 1 : quantity?.toString()?.length + 2}ch`
                    }} value={quantity} onChange={handleChange} onBlur={handleBlur} />
                      </div>

                      <div className="increment_decrement_button">
                        <div className="increment_decrement_button__increment" onClick={increment}>
                          <img className="w-full" src={IncrementButtonArr} alt="" />
                        </div>
                        <div className="increment_decrement_button__increment" onClick={decrement}>
                      ...
                      <div className="input_go_button_parent__input_div__parent_full">
                        ...
                        <p>
                          ...
                          <span className="price_tag_currency">
                            ...
                            {Number(quantity * itemData?.price * (itemData?.chainId == 137 ? maticBal : ethBal)).toFixed(6)}
                            ){" "}
                          </span>
                        </p>
                      </div>
                ...
                {!activeButton && <div>
                    <button className="go_button" onClick={() => {
              ...
              {!activeButton ? <div className="total-price mt-3">
