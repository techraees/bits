...
const NavbarComponent = ({
  ...
  const nonNavbarRoutes = ["/login", "/signup", "/reset-password", "/reset-password/success", "/404"];
  const handleMenu = () => {
    setMenuBar(!menuBar);
  };
  ...
  const effectiveWalletChainId = liveWalletChainId ?? (walletChainId != null ? Number(walletChainId) : null);
  useEffect(() => {
  ...
  const handleChainSelect = async targetChain => {
    ...
    const isMismatched = effectiveWalletChainId != null && effectiveWalletChainId !== targetChain;
    if (isMismatched) {
  ...
  const isChainMismatched = isLoggedIn && isConnected && effectiveWalletChainId != null && effectiveWalletChainId !== Number(contractData?.chain);
  ...
  const getPageName = pathName => {
    const pageName = routes?.find(route => trimAfterFirstSlash(route?.path) === trimAfterFirstSlash(pathName))?.name;
  ...
  return <>
      <Navbar className={`dashboardNavBgColor ${dashboardNav ? headerTheme || "dashboardNavBgColor" : headerTheme || "navbarBgColor"}`} expand="lg" sticky="top" style={{
      zIndex: 100000000
    }}>
        {isModalOpen && <Modal open={isModalOpen} onOk={handleOk} onCancel={handleCancel} footer={false} className="logoutModal" width={300} centered={width < 992 && true} zIndex={100000200} maskStyle={{
        zIndex: 100000190
        ...
        <Navbar.Brand className="web-nav-brand">
          <img onClick={toggleCollapsed} src={menu_icon} className="cursor ms-4 menuBarWebView web-nav" alt="menu-icon" />
        </Navbar.Brand>
        <Container>
          {full_name && <img onClick={handleMenu} src={menu_icon} className="cursor ms-4 mob-nav " alt="icon" />}
          <Navbar.Brand>
            <div className="nav-logo">
              <NavLink to="/" className="white d-flex">
                <img src={logo} className="cursor" style={{
                width: 50,
                height: 50
              }} alt="logo" />
              </NavLink>
            ...
            <div className="d-flex align-items-center justify-content-center loginbtn">
              {!full_name ? <>
                  <div className="connentbtn">
                    ...
                    <Nav.Link className="white mx-2  walletBtn d-flex justify-content-center align-items-center" onClick={handleLogin}>
                      <span>
                        {}
                      </span>
                    </Nav.Link>
                  ...
                  <Nav.Link className="white mx-1 d-flex" onClick={() => showModal()}>
                    ...
                    <img src={validImage ? `${environment.REACT_APP_BACKEND_BASE_URL}/${userProfile}` : profileimg} width={30} style={{
                  borderRadius: "50%"
                }} alt="profile" />
                  </Nav.Link>
          ...
          <Navbar.Toggle aria-controls="basic-navbar-nav" className={headerTheme === "white-navbar" ? "toggle-light" : "toggle-dark"} />
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className={"d-flex align-items-center justify-content-between first-nav navBarPadding"}>
              <NavLink to="/" className="white d-flex  mobile_navlink no_padding_top">
                <img src={home} className="mx-2" style={{
                width: "16px",
                height: "22px"
              }} alt="home" />
                <span>Home</span>
              </NavLink>
              <Nav.Link className="white mobile_navlink text-nowrap" onClick={() => navigate("/marketplace")}>
                Marketplace
              </Nav.Link>
            ...
            <div className={`chainDiv ${isLight ? "chain-light" : "chain-dark"}`}>
              <div className="leftChainDiv">
                ...
                {isChainMismatched && <Tooltip title={`Your wallet is on a different network. Click ${contractData?.chain === 1 ? "ETH" : "Polygon"} to switch it.`}>
                    <span className="chain-mismatch-dot" />
                  </Tooltip>}
              </div>
              <div className="rightChainDiv">
                <FaEthereum cursor="pointer" onClick={() => handleChainSelect(1)} className={`chainIcon ${iconClicked ? "red" : ""}`} />
                <img className={`ethIcon ${showRedImage ? "" : "hidden"}`} src={polygon} alt="Polygon" onClick={() => handleChainSelect(137)} width={15} height={15} />
                <img className={`ethIcon red ${showRedImage ? "hidden" : ""}`} src={redPolygon} alt="Red Polygon" onClick={() => handleChainSelect(137)} width={15} height={15} />
              </div>
            </div>
            <Nav className="ms-auto bottom-nav">
              {!full_name ? <div className="d-flex align-items-center justify-content-center navbar-menu1">
                  <Nav.Link className="white mx-2" onClick={handleLogin}>
                    Sign up/Login
                  </Nav.Link>
                  <Nav.Link className="white mx-2 walletBtn d-flex justify-content-center align-items-center" onClick={handleLogin}>
                  ...
                  <Nav.Link className="white mx-1 d-flex" onClick={() => showModal()}>
                    ...
                    {profileimg ? <img src={imgPath} width={30} className="" style={{
                  borderRadius: "50%"
                }} alt="profile" onError={e => {
                  e.target.src = profileimg;
                }} /> : <img src={profileimg} width={30} className="" style={{
                  borderRadius: "50%"
                }} alt="profile" />}
                  </Nav.Link>
                  <Nav.Link className="white">
      ...
      {menuBar && <MenuComponent toggleCollapsed={toggleCollapsed} menuHandle={menuBar ? true : collapsed} className=" menuBarWebView   mobile-slide-menu" />}

      <MenuComponent toggleCollapsed={toggleCollapsed} menuHandle={collapsed} className="menuBarWebView mobile-slide-menu web-nav" />

      {getPageName(location?.pathname) && <div className={`${headerTheme} p-2`} style={{
      textAlign: "center"
    }}>
      ...
      <div>
        <CookieConsent location="bottom" buttonText="Accept Cookies" cookieName="cookieConsentCookie" style={{
        background: "#F3F3F3",
        alignItems: "center"
      }} buttonStyle={{
        color: "#ffffff",
