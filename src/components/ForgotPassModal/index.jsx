...
const ForgotPassModal = ({
  ...
  const handleSubmitReset = async () => {
    if (validateEmail(formValue.email)) {
      ...
      const response = await fetch(`${env.REACT_APP_BACKEND_BASE_URL}/forgot-password`, {
        method: "POST",
        headers: headers,
  ...
  return <>
      <Modal style={{
      marginTop: "6rem"
    }} footer={null} open={visible} onOk={onClose} onCancel={onClose}>
        <div className="my-5">
          {step === 1 && <form autoComplete="off">
              <div className="w-100 semi-bold fs-5 text-center text-white mb-3">
                Forgot your password?
              </div>
              <InputComponent placeholder={"E-mail"} name="email" onChange={handleChange} value={formValue?.email} autoComplete="off" />

              <div className="my-3">
                <ButtonComponent onClick={handleSubmitReset} text={"RESET YOUR PASSWORD"} />
              </div>
            </form>}

          {}

          {step === 2 && <div className="w-100 semi-bold fs-5 text-center text-white mb-3">
              Password reset link sent to your email!
            </div>}

          {step === 3 && <form autoComplete="off">
              <div className="w-100 semi-bold fs-5 text-center text-white mb-3">
                Reset your password
              </div>
              <div className="my-3">
                <InputComponent placeholder={"Password"} name="password" onChange={handleChange} value={formValue?.password} autoComplete="off" password />
              </div>

              <PasswordRequirements password={formValue?.password || ""} />

              <div className="my-3">
                <InputComponent placeholder={"Confirm Password"} name="confirm_password" onChange={handleChange} value={formValue?.confirm_password} autoComplete="off" password />
              </div>

              <PasswordRequirements password={formValue?.confirm_password || ""} />

              <div className="my-3">
                <ButtonComponent onClick={handleSubmitConfirm} text={"RESET YOUR PASSWORD"} />
              </div>
            </form>}
        </div>
      </Modal>
