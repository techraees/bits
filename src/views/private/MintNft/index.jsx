...
import { ButtonComponent, LabelInput, Loader, ToastMessage } from "../../../components";
...
import { CREATE_NFT, SEND_EMAIL_MUTATION, CREATE_NEW_TRANSACTION } from "../../../gql/mutations";
...
const getMintedTokenIdFromReceipt = (receipt, contract) => {
  ...
  const logsForContract = receipt.logs.filter(log => log.address?.toLowerCase() === contractAddress);
...
const MintNft = () => {
  ...
  const {
    createNft
  } = useSelector(state => state.nft.createNft);
  const [CreateNft, {
    data,
    loading,
    error
  }] = useMutation(CREATE_NFT);
  ...
  useEffect(() => {
    const sendMsg = async () => {
      const msgData = mintMessage(createNft && createNft.artist_name1, createNft && createNft.name);
      try {
  ...
  const mintCall = async (supply, royalty) => {
    if (!isConnected) {
      ToastMessage("Error", `Please Connect Meta mask with site first`, "error");
      return;
    }
    if (!metamaskAddress) {
      ToastMessage("Error", `AppKitAccount Wallet Address Not Found` + (metamaskAddress || "."), "error");
      return;
    }
    if (metamaskAddress?.toLowerCase() !== userData?.address?.toLowerCase()) {
      ToastMessage("Error", `Profile Wallet Address(${userData?.address}) mismatch with metamask wallet address(${metamaskAddress})`, "error");
      return;
    }
    ...
    const walletSignerProvider = new ethers.providers.Web3Provider(walletProvider);
    ...
    let tx;
    try {
      tx = await contractWithsigner.mint(address, supply, createNft.meta, royalty, splitOwners, splitOwnersPercentage, []);
    } catch (error) {
      const parsedEthersError = getParsedEthersError(error);
      ToastMessage("Error", parsedEthersError.context == -32603 ? "Insufficient Balance" : `${parsedEthersError.errorCode || parsedEthersError.context || "Transaction was not signed"}`, "error");
      throw error;
    }
    setLoadingStatus(true);
    setLoadingMessage("Minting...");
    try {
      const receipt = await readOnlyProvider.waitForTransaction(tx.hash, 1, 5 * 60 * 1000);
      if (receipt.status === 0) {
      ...
      if (!newTkId) {
        throw new Error(`Mint transaction confirmed (${transactionHash}) but token id could not be read from the receipt`);
      }
      return {
        newTkId,
        transactionHash
      ...
      const parsedEthersError = getParsedEthersError(error);
      ToastMessage("Error", parsedEthersError.context == -32603 ? "Insufficient Balance" : `${parsedEthersError.errorCode || parsedEthersError.context || error.message}`, "error");
      throw error;
    } finally {
  ...
  const {
    ...
    onSubmit: async values => {
      ...
      let mintResult;
      try {
        mintResult = await mintCall(Number(values.supply), Number(values.royalty * 100));
      } catch (error) {
        return;
      }
      const {
      ...
      try {
        const nftResponse = await CreateNft({
          variables: {
            ...
            rid: createNft && createNft.download.rid ? createNft.download.rid : "rid",
        ...
        ToastMessage("Error", `Your NFT was minted on-chain, but we couldn't save it to your account. Please contact support with this transaction hash: ${transactionHash}`, "error");
      }
    }
  });
  useEffect(() => {
  ...
  const isRoyaltyReady = values.royalty !== "" && !Number.isNaN(Number(values.royalty)) && Number(values.royalty) >= 0 && Number(values.royalty) <= 100;
  ...
  return <div className={`${backgroundTheme}`} style={{
    minHeight: "100vh"
      ...
      <CreatorEarningModal isOpen={creatorEarningModal} onRequestClose={closeCreatorEarningModel} setSplitOwners={setSplitOwners} setSplitOwnersPercentage={setSplitOwnersPercentage} address={address} />
      ...
      <div className="container py-3 mint-nft-page">
        ...
        <div className={`searchStyle mint-nft-card ${bgColor} my-4 p-5`}>
          <Row style={{
          width: "100%"
        }} className="mint-nft-main-row">
            <Col lg={14} sm={24} xs={24} className="borderBottom">
              <Row className="mint-nft-inner-row" gutter={{
              xs: 0,
              sm: 0,
              md: 30,
              lg: 50
                ...
                <Col lg={12} sm={24} md={12} xs={24}>
                  <div className="cardContainer mintCardContainer mint-nft-player" style={{
                  width: "100%"
                }}>
                    <ReactPlayer controls={true} width="100%" height={220} url={createNft && createNft.video} />
                  </div>
                  <div style={{
                    ...
                    <div className="my-3">
                      <div className="d-flex align-items-center label-input royalty-row">
                        ...
                        <div className="royalty-input-wrap">
                          <Input name="royalty" className={"royaltyInputField"} placeholder={"royalty"} onChange={e => {
                          if (e.target.value < 101) {
                            setFieldValue("royalty", e.target.value);
                          } else {
                            ToastMessage("Error", "Royalty should be less than 100", "error");
                          }
                        }} onKeyDown={e => {
                          if (e.key === "-" || e.key === "+" || e.key === "*" || e.key === "/" || e.key === "e") {
                            e.preventDefault();
                          }
                        }} onWheel={event => event.currentTarget.blur()} type="number" value={values.royalty} autoComplete="off" />
                        </div>
                      </div>
                      <ErrorMessage message={touched.royalty && errors.royalty ? errors.royalty : null} />
                    </div>
                    <div style={{
                    border: "1px solid  #B23232",
                    cursor: "pointer"
                  }} className="p-1 mt-5 text-center rounded-3 red-background">
                      <span className={`${textColor2}`} onClick={handleSplitOwnership}>
                        Split Ownership
                      </span>
                    </div>
                  </div>
            ...
            <Col lg={10} sm={24} xs={24}>
              <div className="supplyView">
                ...
                border: "1px solid  #B23232"
                  ...
                  <span className={`${textColor2}`} onClick={() => navigate(`/collections/${userData?.id}`)}>
                    Go to Collection
                  </span>
                </div>
              </div>
        ...
        width: "100%"
          ...
          <Row>
            ...
            <Col lg={20} md={14} sm={12} xs={24}>
              <Input name="supply" className={"royaltyInputField mint-nft-supply-input"} placeholder={"Enter Quantity To Mint"} onChange={e => {
              setFieldValue("supply", e.target.value);
            }} onKeyDown={e => {
              if (e.key === "-" || e.key === "+" || e.key === "*" || e.key === "/" || e.key === "e") {
                e.preventDefault();
              }
            }} onWheel={event => event.currentTarget.blur()} type="number" value={values.supply} autoComplete="off" />
              <ErrorMessage message={touched.supply && errors.supply ? errors.supply : null} />
            </Col>
          </Row>
          <form>
            <LabelInput borderColor={"#B23232"} placeholder={values?.walletAddress} name="walletAddress" value={values.walletAddress} onChange={e => {
        ...
        <div className="mint-checklist mb-3">
          ...
          <ul className="mint-checklist__list">
            <li className={`mint-checklist__item ${isSupplyReady ? "mint-checklist__item--done" : ""}`}>
              <FaCheckCircle className="mint-checklist__icon" size={14} />
              <span>Circulating supply entered</span>
            </li>
            <li className={`mint-checklist__item ${isRoyaltyReady ? "mint-checklist__item--done" : ""}`}>
              <FaCheckCircle className="mint-checklist__icon" size={14} />
              <span>Royalty percentage set (0-100%)</span>
            </li>
            <li className={`mint-checklist__item ${isWalletReady ? "mint-checklist__item--done" : ""}`}>
              <FaCheckCircle className="mint-checklist__icon" size={14} />
        ...
        <div className="d-flex align-items-center justify-content-center mint-nft-actions">
          <div className="mint-nft-action-btn">
            <Button className="px-5 cancelBtn" style={{
          ...
          <div className="mint-nft-action-btn">
            <div className={!isMintReady ? "mint-btn-disabled" : ""} title={!isMintReady ? "Complete the checklist above before minting" : undefined}>
              <ButtonComponent onClick={handleSubmit} text={"Mint NFT"} width={150} height={40} />
            </div>
          </div>
