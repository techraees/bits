...
import { profile_large, location, upload, search, AZ, grid, profile, location_dark, upload_red } from "../../../assets";
...
const SellsNft = () => {
  ...
  const {
    nftAddress
  } = useSelector(state => state.address.nftAddress);
  ...
  const sortingHandle = () => {
    if (isSorting) {
      ...
      if (isSearching) {
        let data = nfts?.filter(item => item.name.toLowerCase().startsWith(isSearching));
        setIsSearch(data);
      } else {
        setAllNfts(allNfts?.getAllNfts);
      }
      ...
      <div className="container">
            ...
            <div className="d-flex justify-content-center my-3 align-items-center flex-wrap">
              ...
              <div className="ms-3">
                ...
                <div className="d-flex mb-1">
                  ...
                  <img className="ms-2" src={textColor === "white" ? location : location_dark} />
                </div>
                <span className={`${textColor2}`}>
                  (Snap Boggie is a Professional Dancer){" "}
                </span>
                <div className="mt-2">
                  <ButtonComponent onClick={() => navigate("/account-settings/edit-profile")} simple text={"Edit Profile"} width={150} />
                </div>
              </div>
            </div>
          </Col>
        ...
        <div className="my-4 d-flex justify-content-between">
          ...
          width: "100%"
            ...
            let data = nfts?.filter(item => item.name.toLowerCase().startsWith(e.target.value));
            setIsSearch(data);
          }} />
        ...
        <div className="tabsWrapper">
          <Tabs defaultActiveKey="1">
            ...
            <Tabs.TabPane tab="NFT’s Owned" key="2" className={textColor == "black" && "ant-light"}></Tabs.TabPane>
          </Tabs>
        ...
        <div className="row">
          {cardsData && isSearching ? cardsData?.map((e, i) => {
          return <CardCompnent key={i} image={e.image} status={e.status} name={e.name} videoLink={e.video} sellnft />;
